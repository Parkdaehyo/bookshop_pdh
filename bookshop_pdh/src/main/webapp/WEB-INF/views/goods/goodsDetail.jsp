...
<c:set var="goods"  value="${goodsMap.goodsVO}"  /> <!--  GoodsController의  goodsDetail.do 메서드가 goodsMap을 전달. -->
<c:set var="imageList"  value="${goodsMap.imageList }"  />
 <%
     //치환 변수 선언합니다.
      //pageContext.setAttribute("crcn", "\r\n"); //텍스트 개행문자를 변수 crcn으로 대체합니다.
      pageContext.setAttribute("crcn" , "\n"); 
      pageContext.setAttribute("br", "<br/>"); //br 태그를 변수 br로 대체합니다.
%>  
<html>
<head>
<style>
...
<script type="text/javascript">
	function add_cart(goods_id) {
		$.ajax({
			...
			}, //이 값을 가지고 addGoodsInCart.do을 실행한 후에, success가 작동이됩니다.
			success : function(data, textStatus) {
				//alert(data);
			//	$('#message').append(data);
				if(data.trim()=='add_success'){ //문자열 좌우에서 공백을 제거하는 함수가 trim() 함수 입니다.
...
<body>
	...
	<div id="detail_table">
		<table>
			...
			<!-- 	<tr class="dot_line">
				...
				<tr>
					...
					<td class="fixed"><strong>[당일배송]</strong> 당일배송 해드려요!<br> <strong>[해외배송]</strong>
						해외배송 해드려요!</TD>
				</tr>
				<tr>
					<td class="fixed">도착예정일</td>
					<td class="fixed">이 상품은 2~3일정도 소요될 수 있습니다.</td>
				</tr>
				<tr>
		...
		<ul>
			<li><a href="javascript:fn_order_each_goods('${goods.goods_id }','${goods.goods_title }','${goods.goods_sales_price}','${goods.goods_fileName}');">구입하기 </a></li>
			<li><a class="cart" href="javascript:add_cart('${goods.goods_id }')">장바구니</a></li>
			
			<li><a class="wish" href="#">쿠폰함</a></li>
		</ul>
	</div>
	<div class="clear"></div>
	<!-- 내용 들어 가는 곳 -->
	<div id="container">
		...
		<div class="tab_container">
			<div class="tab_content" id="tab1">
				<h4>책소개</h4>
				<p>${fn:replace(goods.goods_intro,crcn,br)}</p> <!--  줄개행 crcn을 br(엔터)로 바꾸겠다. -->
			...
			<div class="tab_content" id="tab2">
				...
				<%-- replace함수를 이용해 저자 소개에 포함된 crcn(개행문자)을 br<br> 태그로 대체합니다. --%>
				 <p>${fn:replace(goods.goods_writer_intro,crcn,br) }</p> 
				
			</div>
			<div class="tab_content" id="tab3">
				<h4>책목차</h4>
				<%-- 마찬가지로 상품 목차에 포함된 crcn(개행문자)을 br<br>태글 대체합니다. --%>
				<p>${fn:replace(goods.goods_contents_order,crcn,br)}</p> 
			</div>
			<div class="tab_content" id="tab4">
				<h4>출판사서평</h4>
				<%-- 상품목차에 포함된 crcn(개행문자)을 br<br> 태그로 대체합니다. --%>
				 <p>${fn:replace(goods.goods_publisher_comment ,crcn,br)}</p> 
			</div>
			<div class="tab_content" id="tab5">
				<h4>추천사</h4>
				<%-- 상품 추천평에 포함된 crcn(개행문자)을 br<br>태그로 대체합니다. --%>
